...
void SpeedLimitCalc(s32 wSpeedFbk, stru_PowerBasic_t *pstruPower)
{
    if(++pstruPower->nSpeedLimitCntr >= SPEED_LIMIT_TIME)                     //限转速计算周期
    {
        ...
        if(wSpeedFbk >= pstruPower->wSpeedLimitValue)                        //实际转速大于限制转速则减小功率给定值
        {
            ...
            {
                pstruPower->wSpeedLimitPowerSet += pstruPower->struPowerRamp.nACCStep;
            }
            else
            {
                pstruPower->wSpeedLimitPowerSet = pstruPower->wSpeedLimitPowerRef;
            }
        }
        else                                                                  //转速恢复正常后，减小限转速输出
        {
            ...
            {
                pstruPower->wSpeedLimitPowerSet -= pstruPower->struPowerRamp.nDECStep;
            }
            else
            {
                pstruPower->wSpeedLimitPowerSet = 0;
            }
        }
    }
}
 ...
 * 说明     : 功率计算
* 设计思路  : 1.P = ABS(Id*Ud) + ABS(Iq*Uq) ,计算出来的功率值和实际功率进行曲线拟合，求出斜率和差值 \
...
void PowerCalc( stru_CurrVoctorDQ *pstruCurrDQ, stru_VoltVoctorDQ *pstruVoltDQ , stru_PowerBasic_t *pstruPower)
{
    ...
    pstruPower->struPowerLowPass.wInput = pstruPower->wTemp;
...
/*****************************************************************************
 * 函数名   : DIVCalc(u32 wDividend, s16 nDivisor)
 * 说明     : 除法计算
 * 设计思路 : 1.除法
 * 参数     ：无
 * 返回值   ：无
 * 修改时间 ：2020.08.17
 *****************************************************************************/
u16 DIVCalc(u32 wDividend, s16 nDivisor)
{
    u16 t_nTemp;
...
void PowerLoopReg(stru_PowerBasic_t *this)
{
    ...
    this->struPowerPI.wInError = sat(this->struPowerPI.wInError, -1000, 1000);
