...
typedef struct 
{
    ...
    s32 nDirTrackFreq;              //电机切入闭环频率
    s32 nDirEbreakFreq;             //电机刹车频率
    ...
    u16 nMotorRunDirPre;            //上次电机转向
		...
		s32 wVSPSpeedSet;               //VSP调速时的给定转速
		s32 wSpeedValue;                //速度给定值
		s16 nVspADC;                     //VSP采样电压
		
    s16 nBemfDuty;                  //反电势刹车占空比
    u16 nBemfTrackVolt;             //电机高速反电势电压阈值
    s32 wBemfTrackFreq;
...
typedef struct
{
    ...
    s16 nBemfheckTime;     //bemf顺逆风时间，以1ms为单位
    s16 nBemfBrakeCntr;    //反电势刹车时间，以1ms为单位  
}stru_Time_t;
...
extern void StateBemfDirCheck(void);
extern void StateBemfStop(void);
