...
typedef union
{
    ...
    struct
    {
        u16 SoftCurretError : 1;//软件过流错误标志位   1
        u16 HardCurretError : 1;//硬件过流错误标志位   2
        u16 VoltOverError   : 1;//过压错误标志位       4
        u16 VoltUnderError  : 1;//欠压错误标志位       8
        u16 StallError      : 1;//堵转错误标志位       16
        u16 EmptyError      : 1;//离水空转错误标志位   32
        u16 PhaseLossError  : 1;//缺相错误标志位       64
        u16 TempOverError   : 1;//过温错误标志位       128
        u16 OffsetError     : 1;//偏置错误标志位       256
        u16 StartError      : 1;//启动错误标志位       512
...
extern void FaultCurrentCheck(stru_CurrPhaseUVW *pCurrent);
