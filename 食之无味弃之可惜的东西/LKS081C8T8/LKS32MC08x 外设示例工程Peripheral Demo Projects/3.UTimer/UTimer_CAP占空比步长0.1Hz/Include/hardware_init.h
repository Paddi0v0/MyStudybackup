...
typedef struct
{
	  UINT32 InPut_risitimevalue ;//捕获PWM高电平时间
    UINT32 InPut_falltimevalue; //捕获PWM低电平时间
    UINT32 LEVEL_HL;//PWM恒为高为1，恒为低为0.
	  u16 InPut_DutyCycle;          //占空比

}stru_PWMFreqCat;
extern stru_PWMFreqCat  gS_PWMFreqCat;

#define SPsetup_Voltage  GPIO_ReadInputDataBit(GPIO2,GPIO_Pin_8)
#define RAM_FUNC __attribute__((noinline, used ,section ("ram_functions"))) //RAM区定义
