...
void SPI_init(void)
{
    ...
    SPI_InitStruct.BaudRate = 3;                 //传输速度96MHZ/(2*(30+1))= 1.548MHZ
    SPI_InitStruct.ByteLength = 0;                // 发送零个字节
    //使能中断开关， 使能 SPI传输完成中断;  使能传输异常中断； 使传输溢出中断
    SPI_InitStruct.IRQEna = SPI_IRQEna_Enable | SPI_IRQEna_TranDone | SPI_IRQEna_SSErr | SPI_IRQEna_DataOver; //使能SPI中断
...
void Spi_master_DMA(UINT8 *spi_txdma_data, UINT8 *spi_rxdma_data, u8 spi_len)
{
	  ...
	  spi_len = spi_len + 1;
	  SPI_SIZE = spi_len;                                          //SPI发送spi_len字节后使SPI不进入空闲状态
    ...
    DMA_CTRL = 0x0001;             //DMA使能
		...
		SPI_TX_DATA = 0x000000aa;      //触发SPI传输完成标志位置位，使SPI_DMA搬移，
}
