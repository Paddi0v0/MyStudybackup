...
int ProgramPage_WrtieOnly(unsigned long adr, unsigned long sz, unsigned char *buf)
{
    /* Add your Code */
    volatile unsigned int Addr, size;
    volatile unsigned char *p;
    ...
    while(size)
    {
        ...
        while(1){if(FLASH_READY)break;}// Wait Operation Done!
        p += 4;
        size -= 4;                       // Go to next Word
        Addr += 4;
    }
