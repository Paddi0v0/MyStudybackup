...
void Hardware_init(void)
{
    ...
    CAN_init();                    /* CAN初始化*/
	  ...
	  NVIC_EnableIRQ(CAN_IRQn);      /* 使能CAN中断 */
    NVIC_SetPriority(CAN_IRQn, 0); /* 配置CAN中断优先级 */
...
void CAN_init(void)
{
    ...
    /*接收滤波： ID:0x5A ,标准帧，BYTE1:0x81,BYTE:0x23*/
 ...
 *******************************************************************************/
void CAN_DMA_TX(u8 ide, u8 frame_number, u8 frame_size, u8 *memaddr)
{
    u8 len = 0;
