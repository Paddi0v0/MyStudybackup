...
@note         Overflow is detected. The functions saturates the return
              value if it cannot fit into the return type.
-----------------------------------------------------------------------------*/
static __inline tFrac32 F32MulSatF16F16(register tFrac16 x, register tFrac16 y)
{
	register tS32 m;
...
@note         Overflow is not detected. The first argument must be lower in
              ...
              rounded toward zero.
-----------------------------------------------------------------------------*/
static __inline tS32 F32DivNorm(register tU32 x)
{
    register tS32 b;
    register tS32 m;
    register tS32 mm;
