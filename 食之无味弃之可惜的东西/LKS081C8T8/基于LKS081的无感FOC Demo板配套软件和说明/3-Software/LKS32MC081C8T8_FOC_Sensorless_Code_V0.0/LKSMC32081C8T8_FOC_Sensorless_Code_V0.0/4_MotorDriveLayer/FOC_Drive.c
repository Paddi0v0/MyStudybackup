	...
	switch(t_eMotorStatus)
	{
		...
		{			
			...
			//if(E_PWM_ON == tS_pMotorFoc->ePWMStatusAck)
			{
				...
				#else
				DSP_SC |= BIT2;
    			DSP_CORDIC_THETA = (tS_pMotorFoc->m_pMotorAngle->wElectAngle >> 16);                           /* sincos通过DSP计算 */
				tS_pMotorFoc->mSinCosFocAngle.hCos	= DSP_CORDIC_COS;
				tS_pMotorFoc->mSinCosFocAngle.hSin	= DSP_CORDIC_SIN;				
				#endif
