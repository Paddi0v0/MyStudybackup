...
void ADC_Init(ADC_TypeDef* ADCx, ADC_InitTypeDef* ADC_InitStruct)
{
    ...
    t_reg = (u16)((ADC_InitStruct->Align) | (u16)(ADC_InitStruct->DAT0_TH_Type << 1));
    ADCx->CFG = (u16)t_reg;

    t_reg = (u16)(ADC_InitStruct->MCPWM_Trigger_En) | (u16)(ADC_InitStruct->UTIMER_Trigger_En << 4) |
            (u16)(ADC_InitStruct->Trigger_Cnt << 8) | (u16)(ADC_InitStruct->Trigger_Mode << 12) |
            (u16)(ADC_InitStruct->Con_Sample << 14);

    ADCx->TRG = (u16)t_reg;//TRIG

    ADCx->CHNT0 = (u16)(ADC_InitStruct->FirSeg_Ch | (ADC_InitStruct->SecSeg_Ch << 8));
    ADCx->CHNT1 = (u16)(ADC_InitStruct->ThrSeg_Ch | (ADC_InitStruct->FouSeg_Ch << 8));
...
void ADC_ChannelConfig(ADC_TypeDef* ADCx,CHNx CHNum, uint16_t nChannel)
{
    volatile u32 *p;
	u32 reg;
	p = (volatile u32*)&ADCx->CHN0;
	reg = p[CHNum >> 1];
	reg &= ~(u32)(0xff << (8*(CHNum&1)));
	reg |= (u32)nChannel << (8*(CHNum&1));
	p[CHNum >> 1] = reg;
}
/*******************************************************************************
