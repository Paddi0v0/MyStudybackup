...
int ProgramPage(u32 adr, u32 sz, u8 *buf, u16 nvr)
{
    ...
    if(progm_flag == 0x9AFDA40C)
    {
      ...
      p    = buf;
      size = (sz + 3) & ~(u32)3;                  // align the word
      Addr = adr;
      while(size)
      {
        FLASH_ADDR = (Addr & ~(u32)3);   // address alignment
        FLASH_WDATA= (u32)(*p + (u32)(*(p+1)<<8) + (u32)(*(p+2)<<16) + (u32)(*(p+3)<<24));            // write data to flash
      ...
      while(size)
      {
        u32 t_RData;
        FLASH_ADDR = (Addr & ~(u32)3);   // address alignment
        t_RData = (u32)*p + (u32)(*(p+1)<<8) + (u32)(*(p+2)<<16) + (u32)(*(p+3)<<24);
...
/*******************************************************************************
 函数名称：    void Read_Flash(uint32_t adr , u16 nvr)
 功能描述：    读取FLASH数据函数
 输入参数：    adr：要读取数据的地址  
