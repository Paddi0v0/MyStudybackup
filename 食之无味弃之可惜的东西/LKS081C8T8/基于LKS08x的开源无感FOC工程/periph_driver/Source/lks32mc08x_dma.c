...
void DMA_Init(DMA_RegTypeDef *DMAx, DMA_InitTypeDef *DMAInitStruct)
{
    /* 通道配置寄存器 斌值 */
    DMAx->DMA_CCR = 0;
    DMAx->DMA_CTMS = (u32)DMAInitStruct->DMA_TIMES | (u32)(DMAInitStruct->DMA_ROUND<<16); /* 传输次数寄存器 */
    DMAx->DMA_CPAR = DMAInitStruct->DMA_CPAR;   /* 外设地址寄存器 */
    DMAx->DMA_CMAR = DMAInitStruct->DMA_CMAR;   /* 内存地址寄存器 */
    DMAx->DMA_CCR  =  (u32)(DMAInitStruct->DMA_IRQ_EN<<1) | (u32)(DMAInitStruct->DMA_DIR<<4) |\
                     (u32)(DMAInitStruct->DMA_CIRC<<5) | (u32)(DMAInitStruct->DMA_PINC<<6) | (u32)(DMAInitStruct->DMA_MINC<<7) |\
                     (u32)(DMAInitStruct->DMA_PBTW<<8) | (u32)(DMAInitStruct->DMA_MBTW<<10) | (u32)(DMAInitStruct->DMA_REQ_EN<<12);
...
/*******************************************************************************
 函数名称：    void DMA_ClearIRQFlag(uint32_t tempFlag)
 功能描述：    清除DMA中断标志
 操作的表：    无
 输入参数：    无
 输出参数：    无
