...
typedef struct {
  ...
  uint16_t MCPWM_WorkModeCH0; /**< MCPWM CH0工作模式：边沿对齐/中心对齐 @see
                                 EDGE_PWM_MODE  @see CENTRAL_PWM_MODE*/
  uint16_t MCPWM_WorkModeCH1; /**< MCPWM CH1工作模式：边沿对齐/中心对齐 @see
                                 EDGE_PWM_MODE  @see CENTRAL_PWM_MODE*/
  uint16_t MCPWM_WorkModeCH2; /**< MCPWM CH2工作模式：边沿对齐/中心对齐 @see
                                 EDGE_PWM_MODE  @see CENTRAL_PWM_MODE*/
  uint16_t MCPWM_WorkModeCH3; /**< MCPWM CH3工作模式：边沿对齐/中心对齐 @see
                                 EDGE_PWM_MODE  @see CENTRAL_PWM_MODE*/

  uint16_t
      TriggerPoint0; /**<
                        PWM触发ADC事件0，时间点设置，取值范围（-MCPWM_PERIOD~MCPWM_PERIOD）
                      */
  uint16_t
      TriggerPoint1; /**<
                        PWM触发ADC事件1，时间点设置，取值范围（-MCPWM_PERIOD~MCPWM_PERIOD）
                      */
  uint16_t
      TriggerPoint2; /**<
                        PWM触发ADC事件2，时间点设置，取值范围（-MCPWM_PERIOD~MCPWM_PERIOD）
                      */
  uint16_t
      TriggerPoint3; /**<
                        PWM触发ADC事件3，时间点设置，取值范围（-MCPWM_PERIOD~MCPWM_PERIOD）
                      */
  /**
   * @brief CH0N死区时间设置 \n
   *   #define DEADTIME (u16)(((unsigned long long)PWM_MCLK * (unsigned long
   * long)DEADTIME_NS)/1000000000uL) \n
  ...
  uint8_t CH0P_SCTRLP; /**< 当CH0_PS =
                          1时，输出到CH0_P通道的值高电平(前提极性没有取反)*/
  uint8_t CH0N_SCTRLN; /**< 当CH0_NS =
                          1时，输出到CH0_N通道的值高电平(前提极性没有取反)*/
  uint8_t CH1P_SCTRLP; /**< 当CH1_PS =
                          1时，输出到CH1_P通道的值高电平(前提极性没有取反)*/
  uint8_t CH1N_SCTRLN; /**< 当CH1_NS =
                          1时，输出到CH1_N通道的值高电平(前提极性没有取反)*/
  uint8_t CH2P_SCTRLP; /**< 当CH2_PS =
                          1时，输出到CH2_P通道的值高电平(前提极性没有取反)*/
  uint8_t CH2N_SCTRLN; /**< 当CH2_NS =
                          1时，输出到CH2_N通道的值高电平(前提极性没有取反)*/
  uint8_t CH3P_SCTRLP; /**< 当CH3_PS =
                          1时，输出到CH3_P通道的值高电平(前提极性没有取反)*/
  uint8_t CH3N_SCTRLN; /**< 当CH3_NS =
                          1时，输出到CH3_PN通道的值高电平(前提极性没有取反)*/
  ...
  uint8_t
      CH2_PS; /**<
                 CH2_P信号来源，1：来自CH2_SCTRLP（软件控制PWM输出）；0：来自MCPWM实际运行计数器（硬件计数器控制PWM输出）*/
  uint8_t
      CH2_NS; /**<
                 CH2_N信号来源，1：来自CH2_SCTRLN（软件控制PWM输出）；0：来自MCPWM实际运行计数器（硬件计数器控制PWM输出）*/
  uint8_t
      CH3_PS; /**<
                 CH3_P信号来源，1：来自CH3_SCTRLP（软件控制PWM输出）；0：来自MCPWM实际运行计数器（硬件计数器控制PWM输出）*/
  uint8_t
      CH3_NS; /**<
                 CH3_N信号来源，1：来自CH3_SCTRLN（软件控制PWM输出）；0：来自MCPWM实际运行计数器（硬件计数器控制PWM输出）*/

  uint8_t Switch_CH0N_CH0P; /**< 交换CH0N,
                               CH0P信号输出使能开关：ENABLE使能，DISABLE关闭*/
  uint8_t Switch_CH1N_CH1P; /**< 交换CH1N,
                               CH1P信号输出使能开关：ENABLE使能，DISABLE关闭*/
  uint8_t Switch_CH2N_CH2P; /**< 交换CH2N,
                               CH2P信号输出使能开关：ENABLE使能，DISABLE关闭*/
  uint8_t Switch_CH3N_CH3P; /**< 交换CH3N,
                               CH3P信号输出使能开关：ENABLE使能，DISABLE关闭*/

  uint8_t
      MCPWM_UpdateInterval; /**< MCPWM
                               T0/T1事件更新间隔，0~15对应发送更新事件0~15次才更新一次
                             */
  uint8_t
      MCPWM_T0_UpdateEN; /**< MCPWM T0事件更新使能：ENABLE使能，DISABLE关闭*/
  uint8_t
      MCPWM_T1_UpdateEN; /**< MCPWM T1事件更新使能：ENABLE使能，DISABLE关闭*/
  uint8_t
      MCPWM_Auto_ERR_EN; /**<
                            当发生FAIL事件关闭MCPWM时（即MOE关闭），MCPWM更新事件是否自动打开MOE,
                            使能开关 */

  uint8_t FAIL0_INPUT_EN;    /**< FAIL0 输入功能使能：ENABLE使能，DISABLE关闭 */
  uint8_t FAIL1_INPUT_EN;    /**< FAIL1 输入功能使能：ENABLE使能，DISABLE关闭 */
  uint8_t FAIL0_Signal_Sel;  /**< FAIL0 信号选择，比较器0 或 GPIO(MCPWM_BKIN0)
                                @see FAIL_SEL_CMP @see FAIL_SEL_IO */
  uint8_t FAIL1_Signal_Sel;  /**< FAIL1 信号选择，比较器1 或 GPIO(MCPWM_BKIN1)
                                @see FAIL_SEL_CMP @see FAIL_SEL_IO */
  uint8_t FAIL0_Polarity;    /**< FAIL0 信号极性设置，高有效或低有效 @see
                                HIGH_LEVEL_ACTIVE @see LOW_LEVEL_ACTIVE */
  uint8_t FAIL1_Polarity;    /**< FAIL1 信号极性设置，高有效或低有效 @see
                                HIGH_LEVEL_ACTIVE @see LOW_LEVEL_ACTIVE */
  uint8_t DebugMode_PWM_out; /**< Debug时，MCU进入Halt, MCPWM信号是否正常输出 */

  uint8_t
      CH0P_default_output; /**< CH0P MOE为0时或发生FAIL事件时，默认电平输出,@see
                              HIGH_LEVEL @see LOW_LEVEL */
  uint8_t
      CH0N_default_output; /**< CH0N MOE为0时或发生FAIL事件时，默认电平输出,@see
                              HIGH_LEVEL @see LOW_LEVEL */
  uint8_t
      CH1P_default_output; /**< CH1P MOE为0时或发生FAIL事件时，默认电平输出,@see
                              HIGH_LEVEL @see LOW_LEVEL */
  uint8_t
      CH1N_default_output; /**< CH1N MOE为0时或发生FAIL事件时，默认电平输出,@see
                              HIGH_LEVEL @see LOW_LEVEL */
  uint8_t
      CH2P_default_output; /**< CH2P MOE为0时或发生FAIL事件时，默认电平输出,@see
                              HIGH_LEVEL @see LOW_LEVEL */
  uint8_t
      CH2N_default_output; /**< CH2N MOE为0时或发生FAIL事件时，默认电平输出,@see
                              HIGH_LEVEL @see LOW_LEVEL */
  uint8_t
      CH3P_default_output; /**< CH3P MOE为0时或发生FAIL事件时，默认电平输出,@see
                              HIGH_LEVEL @see LOW_LEVEL */
  uint8_t
      CH3N_default_output; /**< CH3N MOE为0时或发生FAIL事件时，默认电平输出,@see
                              HIGH_LEVEL @see LOW_LEVEL */
  ...
  uint8_t
      TH00_Match_INT_EN; /**<
                            TH00与计数器值匹配事件中断使能：ENABLE使能，DISABLE关闭
                          */
  uint8_t
      TH01_Match_INT_EN; /**<
                            TH01与计数器值匹配事件中断使能：ENABLE使能，DISABLE关闭
                          */
  uint8_t
      TH10_Match_INT_EN; /**<
                            TH10与计数器值匹配事件中断使能：ENABLE使能，DISABLE关闭
                          */
  uint8_t
      TH11_Match_INT_EN; /**<
                            TH11与计数器值匹配事件中断使能：ENABLE使能，DISABLE关闭
                          */
  uint8_t
      TH20_Match_INT_EN; /**<
                            TH20与计数器值匹配事件中断使能：ENABLE使能，DISABLE关闭
                          */
  uint8_t
      TH21_Match_INT_EN; /**<
                            TH21与计数器值匹配事件中断使能：ENABLE使能，DISABLE关闭
                          */
  uint8_t
      TH30_Match_INT_EN; /**<
                            TH30与计数器值匹配事件中断使能：ENABLE使能，DISABLE关闭
                          */
  uint8_t
      TH31_Match_INT_EN; /**<
                            TH31与计数器值匹配事件中断使能：ENABLE使能，DISABLE关闭
                          */

  uint8_t
      TMR0_Match_INT_EN; /**<
                            TMR0与计数器值匹配事件中断使能：ENABLE使能，DISABLE关闭
                          */
  uint8_t
      TMR1_Match_INT_EN; /**<
                            TMR1与计数器值匹配事件中断使能：ENABLE使能，DISABLE关闭
                          */
  uint8_t
      TMR2_Match_INT_EN; /**<
                            TMR2与计数器值匹配事件中断使能：ENABLE使能，DISABLE关闭
                          */
  uint8_t
      TMR3_Match_INT_EN; /**<
                            TMR3与计数器值匹配事件中断使能：ENABLE使能，DISABLE关闭
                          */

  uint8_t
      MCPWM_REG_UPDATE_INT_EN; /**<
...
#define MCPWM_SHADE_IRQ_EN                                                     \
  BIT14 /**< MCPWM_TH/MCPWM_TH00-MCPWM_TH31/                                   \
...
#define MCPWM_SHADE_IRQ_IF                                                     \
  BIT14 /**< MCPWM_TH/MCPWM_TH00-MCPWM_TH31/                                   \
         MCPWM_TR0-MCPWM_TR3等寄存器更新到MCPWM实际运行系统中断标志位 */

#define MCPWM_CH0_SHORT_IRQ_IF                                                 \
  BIT15 /**< MCPWMCH0_P和CH0N同时有效中断标志位                      \
         */
#define MCPWM_CH1_SHORT_IRQ_IF                                                 \
  BIT16 /**< MCPWMCH1_P和CH1N同时有效中断标志位                      \
         */
#define MCPWM_CH2_SHORT_IRQ_IF                                                 \
  BIT17 /**< MCPWMCH2_P和CH2N同时有效中断标志位                      \
         */
#define MCPWM_CH3_SHORT_IRQ_IF                                                 \
  BIT18                          /**< MCPWMCH3_P和CH3N同时有效中断标志位 \
                                  */
