 ...
 */
typedef enum {
    GPIO_Mode_IN  = 0x00, /*!< GPIO Input Mode */                                 /*!< GPIO输入模式 */
    GPIO_Mode_OUT = 0x01, /*!< GPIO Output Mode */                                /*!< GPIO输出模式 */
    GPIO_Mode_ANA = 0x02, /*!< GPIO Analog In/Out Mode,用作ANA时需禁止输入输出 */ /*!< GPIO模拟模式，用作模拟输入/输出时需禁用数字功能 */
    GPIO_Mode_IO  = 0x03
} GPIOMode_TypeDef;
...
typedef struct
{
    uint32_t GPIO_Pin; /*!< 要配置的Pad */                        /*!< 要配置的引脚 */
    GPIOMode_TypeDef GPIO_Mode; /*!< GPIO模式:输入、输出、模拟 */ /*!< GPIO模式：输入、输出、模拟 */
    GPIOPuPd_TypeDef GPIO_PuPd; /*!< 默认/上拉 */                 /*!< 默认/上拉 */
    uint32_t GPIO_PODEna;                                         /*!< 开漏 0是禁用开漏输出,1是启用开漏输出 */
} GPIO_InitTypeDef;
