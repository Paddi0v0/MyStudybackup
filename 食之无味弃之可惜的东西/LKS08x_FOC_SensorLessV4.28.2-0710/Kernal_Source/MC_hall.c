...
//      PID_Flux.hKi_Gain = PID_FLUX_KI_DEFAULT;
    ...
    //HALL的转速变量转化成速度闭环反馈的物理量，与开关频率有关
     //开关频率为16KHZ时，转换系数：16k/65536=0.244140625 *符号位
    this->wSpeedfbkHall = App2CoreFreqTrans(User2AppFreqTrans(this->nRotorFreqDppFir * 0.244140625)) * (this->bHallSpeedDirFlag);
}
...
if(struFOC_CtrProc.bMotorDirtionCtrlPhase == CW)
{ 
    this->nPhaseShiftOffset = S16_300_PHASE_SHIFT;       /* 当前Hall角度偏移值设置，部分电机反转和正转HALL超前角不一样*/
}else{
		this->nPhaseShiftOffset = S16_320_PHASE_SHIFT;       /* 当前Hall角度偏移值设置，部分电机反转和正转HALL超前角不一样*/
}
    ...
    this->stru_HallDppRC.coef = FRAC16(0.6);             /* Hall角速度低通滤波FRAC16(0.2) */
