...
#ifndef U_MOTOR_PP
#define HALL_U_MOTOR_PP         ((u8)5)        /* Hall信号处理Buffer大小，6次求平均 */
#else
#define HALL_U_MOTOR_PP         ((u8)1)        /* Hall信号处理Buffer大小，6次求平均 */
#endif
...
typedef struct
{
    ...
    volatile u8      over_state;            // 霍尔超时状态，当霍尔超时时为1
    volatile u8      table_p;               // 缓冲区，指针，指向当前状态
    volatile s8      circle_data;           // 霍尔一周状态求和
    volatile s8      dir;                   // 霍尔方向，在霍尔变化中断中更新     -1：反转    0：静止  1：正转
    volatile u8      data_table[6*HALL_U_MOTOR_PP];         // 霍尔值缓冲区，用来判断电机转向和霍尔状态
    volatile u16     angle_table[6*HALL_U_MOTOR_PP];        // 霍尔角度表
    volatile s32     circle_width;          // 霍尔一周宽度，根据电机方向计算正负值，在霍尔变化中断中更新
    volatile u32     width_table[6*HALL_U_MOTOR_PP];        // 霍尔宽度缓冲区，用来计算转速与角度
    volatile u16     angle;                 // 霍尔计算出来的电机角度
    volatile u32     circle_width_mean;     // 根据当前速度算出的扇区平均宽度
    volatile u32     last_cnt;              // 上次CNT值
    volatile u8      last_hall_state;       // 上次霍尔状态
    volatile u32     speed;                 // 霍尔求出来的速度
}stru_Hall_buffer;
