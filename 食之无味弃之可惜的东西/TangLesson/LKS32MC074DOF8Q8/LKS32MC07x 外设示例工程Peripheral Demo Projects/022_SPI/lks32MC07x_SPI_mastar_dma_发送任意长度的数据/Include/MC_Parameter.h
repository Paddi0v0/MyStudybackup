...
/* -----------------------------Hardware Parameter---------------------------- */
#define ADC_SUPPLY_VOLTAGE             (1.65)              /* 单位: V  ADC基准电压，3.6或者2.4,大部分应用选择3.6 */
/* 采样匹配电阻 40K欧，200:10=(200/(10.4+40)=3.968) */
#define AMPLIFICATION_GAIN             (360.0/(10.0+40.0))               /* 运放放大倍数 */
#define RSHUNT                         (0.05)             /* 单位: Ω  采样电阻阻值 0.1--0.05 */
...
#define ROTOR_SENSOR_TYPE              ROTOR_HALL_SENSOR  /* 电机位置传感器类型选择 */

/* ------------------------------Rated Parameter------------------------------ */

#define U_RATED_VOLTAGE                (24)               /* 单位:V, 电机额定工作电压，设置为正常工作电压 */
#define U_RATED_CURRENT                (1.0)                /* 单位:A, 电机额定工作电流，相电流最大值 */
#define U_MAX_FREQ                     (200.0)            /* 单位:Hz,  电机最高运行转速 */

/* ------------------------------Motor Parameter------------------------------ */
#define U_MOTOR_PP                     (5.0)              /* 电机极对数 */
#define U_MOTOR_RS                     (0.444273)           /* 单位: Ω 电机相电阻 */
#define U_MOTOR_LD                     (343.893)          /* 单位: uH 电机d轴电感 */
#define U_MOTOR_LQ                     (234.4485)          /* 单位: uH 电机q轴电感 */
...
/* -------------------------------- 顺逆风检测参数------------------------------ */
#define SPEED_TRACK_DELAYTIME          (50)              /* 单位: ms 顺逆风检测时间  */
...
#define MOTOR_STOP_CUR_DIF_THD         (120)               /* 电机停止检测电流差阈值 */
...
#define U_START_CUR_SET_F              (0.15)              /* 单位: A 第一段定位电流 */
#define U_START_CUR_SET_S              (0.3)              /* 单位: A 第二段定位电流 */
#define DC_HOLDTIME_TIME_LENTH         (2)               /* 单位: ms 第一段定位时间 */
...
#define ALIGN_CURRENT_ACC              (0.03)              /* 单位: (1/8)A/ms  定位电流加速调整值  初始位置检测使能后给到最大值，不能超过30，否则数据会溢出。 */
#define ALIGN_CURRENT_DEC              (0.03)              /* 单位: (1/8)A/ms  定位电流减速调整值  初始位置检测使能后给到最大值，不能超过30，否则数据会溢出。 */

/*---------------------------------开环参数------------------------------------*/
#define OPEN_ANGLE_TAG_FREQ            (20.0)             /* 单位：Hz 开环拖动最终频率 */
#define FREQ_ACC                       (20.0)              /* 单位：(1/128)Hz/ms 开环拖动频率加速调整值 */
...
#define PLL_KI_GAIN                    50                 /* PLL_Ki 估算器Ki */
...
#define P_CURRENT_KP                   (6000)             /* 电流环Kp，实际运用的Kp会根据这个值和电机参数计算出最终的Kp */
#define P_CURRENT_KI                   (3000)              /* 电流环Ki，实际运用的Kp会根据这个值和电机参数计算出最终的Ki */

#define AUTO_FW_LIM                    ((s16)2000)        /* 自动弱磁D轴电流限制，Q12格式，最大值 4096 */
...
#define P_ASR_KP                       (20000)             /* 速度环Kp */
#define P_ASR_KI                       (20000)              /* 速度环Ki */

#define IQMAX                          (2.6)                /* 单位:A, 速度环输出最大值 */
#define IQMIN                          (-2.6)               /* 单位:A, 速度环输出最小值 */

#define SPEED_RUN_ACC                  (1)              /* 单位 Hz/ms 速度加减速调整值 */
...
/*------------------------------------FaultDetection---------------------------*/
/* 过流检测参数 */
#define I_PH_OVERCURRENT_FAULT         (30.0)              /* 单位：A 软件过流检测设定值 */
