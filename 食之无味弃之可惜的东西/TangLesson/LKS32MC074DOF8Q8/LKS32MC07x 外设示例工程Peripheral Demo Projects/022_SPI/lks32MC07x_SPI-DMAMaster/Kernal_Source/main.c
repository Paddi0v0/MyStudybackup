...
/*******************************************************************************
 ...
 输出参数：    无
 返 回 值：    无
 其它说明：    配置SPI为DMA主模式，发送8个字节数组，用示波器观察输出 例程芯片对应071
 操作说明，    Debug模式下将“SPISendStartFlag”置一开启DMA搬运（demo板按键Start触发发送）。
 修改日期      版本号          修改人            修改内容
 -----------------------------------------------------------------------------
...
UINT8 Rx_data_tbl[32] = {0};												 //接收数据缓冲区
...
int main(void)
{
    ...
    for(;;)
    {
			 ...
			 if(SPISendStartFlag)//SPI发送数据标志位
			 {
				   SPI0_SIZE = 8;
				   DMA0_CTMS1 = 8;
				   DMA0_CTMS2 = 8;
