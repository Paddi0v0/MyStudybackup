...
u8 DMA_TX[13] = {0x11, 0x11, 0x11, 0x11,0x11,0x11,0x11, 0x11, 0x11,0x11,0x11,0x11,0x11};/*搬运数据定义*/
UINT8 Tx_data_tb2[32] = {0xaa, 0xbb, 0x11, 0x22, 0xcc, 0x44, 0x55, 0x66};
void DMA_init(void)
{
    ...
    DMA_InitStruct.SINC = DISABLE; // 源地址递增使能
    DMA_InitStruct.DINC = ENABLE;// 目的地址递增失能
    ...
    DMA_InitStruct.TMS = 8;
    DMA_InitStruct.RMODE = ENABLE;
    DMA_InitStruct.SADR = (u32)Tx_data_tb2;
...
void SPI_init(void)
{
    ...
    SPI_InitStruct.Mode = SPI_Slave;             //master主机模式
    ...
    SPI_InitStruct.IRQEna = ENABLE;              //关闭SPI各中断
...
void GPIO_init(void)
{
    ...
    GPIO_StructInit(&GPIO_InitStruct);
    GPIO_InitStruct.GPIO_Mode = GPIO_Mode_IN;//GPIO_Mode_OUT; //设置为输出模式
    ...
    GPIO_PinAFConfig(GPIO2, GPIO_PinSource_3, AF5_SPI);  //GPIO2.3复用SPI_CS功能
	  ...
	  GPIO_StructInit(&GPIO_InitStruct);
    GPIO_InitStruct.GPIO_Mode = GPIO_Mode_OUT;//GPIO_Mode_IN;            //设置为输入模式
    GPIO_InitStruct.GPIO_Pin = GPIO_Pin_0;
    GPIO_Init(GPIO1, &GPIO_InitStruct);
    GPIO_PinAFConfig(GPIO1, GPIO_PinSource_0, AF5_SPI); //GPIO1.0复用SPI_DI功能
	
	  GPIO_StructInit(&GPIO_InitStruct);
	  GPIO_InitStruct.GPIO_Mode = GPIO_Mode_IN;//GPIO_Mode_OUT;            //设置为输入模式
